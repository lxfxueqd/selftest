...
(function($ , undefined) {
			...
			this.options.dataSource.data($el.data(), function (items) {
				...
				$.each( items.data, function(index, value) {
					...
					if(value.type === "folder") {
						...
						// 添加参数判断，是否加载全部节点  by eric
						if(self.options.allItems)
						{
							// 加载子节点
							self.populate( $entity.find('.tree-folder-header') );
							// 加载子节点，但不显示子节点
							$entity.find('.tree-folder-content').hide();
						}
						...
						// 增加 selected 属性， 如果存在 并且等于true 则显示被选中的状态  by eric
	...
	$.fn.tree.defaults = {
		multiSelect: false,
		allItems : false,			// 是否生成所有节点  by eric
