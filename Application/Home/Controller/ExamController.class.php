...
class ExamController extends Controller{
    public function index(){
        ...
        $where['sei_teacher_id'] =  session('user.sti_teacher_id');
        $total = D('st_exam_inf')->where($where)->count();
        $Page = new \Think\Page($total, 8);
        $data = D('st_exam_inf')->alias('e')->join('JOIN st_exam_paper p on e.sei_paper_id = p.sep_paper_id')
            ->where($where)->limit($Page->firstRow, $Page->listRows)->order('sei_exam_id desc')->select();
