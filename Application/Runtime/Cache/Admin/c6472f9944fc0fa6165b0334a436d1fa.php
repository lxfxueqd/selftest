...
<div class="row-fluid">
	<h3 class="header blue lighter smaller">
		<form action="<?php echo U('Admin/Paper/index');?>" method="get">
		<div class="row-fluid  dataTables_wrapper">
			<div class="span12" id="m_search_div">
				...
				<input name="paper_id" value="<?php echo ($map["sep_paper_name"]); ?>" type="text" id="search_name" class="pull-right" placeholder="试卷名称"/>
		...
		<tbody>
			<?php foreach ($data as $key => $val): ?>
				<tr>
					...
					<td><?php echo ($val["sep_paper_id"]); ?></td>
					<td>
						<?php echo ($val["sep_paper_name"]); ?>
					</td>
					<td><a><?php echo ($val["sep_question_number"]); ?></a></td>
					<td><?php echo ($val["sep_subject_id"]); ?></td>
					<td><?php echo ($val["sep_chapter_id"]); ?></td>
					<td><?php echo ($val["sep_teacher_id"]); ?></td>
					<td><?php echo ($val["sep_start_time"]); ?></td>
					<td><?php echo ($val["sep_end_time"]); ?></td>
					<td><?php echo ($val["sep_exam_length"]); ?></td>
					<td><?php echo ($val["sep_download_number"]); ?></td>
					<td><?php echo C('user_state')[$val['sep_status']];?></td>
					<td>
						<div class="hidden-phone visible-desktop action-buttons">
							...
							<?php else: ?>
								<a href="javascript:;" class="red tooltip-info no-hover-underline" data-rel="tooltip" data-original-title="禁用" onclick="$.check('<?php echo U('Admin/Paper/deny', array('id'=>$val['id']));?>');">
									<i class="icon-legal bigger-130"></i>
								</a>
							<?php endif ?>
						</div>
