...
<div class="row-fluid">
	<h3 class="header blue lighter smaller">
		<form action="<?php echo U('Admin/Student/index');?>" method="get">
		<div class="row-fluid  dataTables_wrapper">
			<div class="span12" id="m_search_div">
				...
				<input name="student_id" value="<?php echo ($map["ssi_student_id"]); ?>" type="text" id="search_name" class="pull-right" placeholder="学号"/>
		...
		<tbody>
			<?php foreach ($data as $key => $val): ?>
				<tr>
					...
					<td><?php echo ($val["ssi_student_id"]); ?></td>
					<td>
						<?php echo ($val["ssi_username"]); ?>
					</td>
					<td><a><?php echo ($val["ssi_realname"]); ?></a></td>
					<td><?php echo ($val["ssi_email"]); ?></td>
					<td><?php echo ($val["ssi_phonenumber"]); ?></td>
					<td><?php echo ($val["ssi_college_id"]); ?></td>
					<td><?php echo ($val["ssi_major_id"]); ?></td>
					<td><?php echo ($val["ssi_grade"]); ?></td>
					<td><?php echo C('user_state')[$val['ssi_status']];?></td>
					<td>
						<div class="hidden-phone visible-desktop action-buttons">
							<?php if ($val['ssi_status'] == 0): ?>
								<a href="javascript:;" class="blue tooltip-info no-hover-underline" data-rel="tooltip" data-original-title="重新启用" onclick="$.check('<?php echo U('Admin/Student/allow', array('id'=>$val['id']));?>');">
									<i class="icon-legal bigger-130"></i>
								</a>
							<?php else: ?>
								<a href="javascript:;" class="red tooltip-info no-hover-underline" data-rel="tooltip" data-original-title="禁用" onclick="$.check('<?php echo U('Admin/Student/deny', array('id'=>$val['id']));?>');">
									<i class="icon-legal bigger-130"></i>
								</a>
							<?php endif ?>
						</div>
